function [P_HMI_H,P_CA,P_HMI_worst,Fault_mag,Fault_slope,H_M,L_M,L_p_M,L_pp_M,Y]= integrity_monitoring_fault...
    (Px, Phi_M ,H_M ,L_M ,L_p_M ,L_pp_M ,A_k ,Y , P_CA, alpha)
...
Y_k_conc=zeros(n_L*m_F, m_F); % Current IIV covarience (concatenated)

for t= 1:n_L
    [h_t,H_t]= compute_lm_model(LM(:,t));
    H_k(((t-1)*m_F)+1:t*m_F,:)= H_t;
    h_k(((t-1)*m_F)+1:t*m_F,:)= h_t;
    Y_k_conc(((t-1)*m_F)+1:t*m_F,:)= H_t * Px * H_t' + PARAMS.R;
end
...
if isempty(A_k)
    ...
    for i= 1:M
        ...
        A_k(:, n_L*m_F*i + 1 : n_L*m_F*(i+1) )=...
            Dummy_Variable * L_M( i*PARAMS.m+1 : (i+1)*PARAMS.m, : );
    end
    A_k( :,n_L*m_F*(M+1)+1 : end )=...
    ...
    for i= 1:M
        ...
        B_bar(i*n_L*m_F+1:(i+1)*n_L*m_F,:)= ...
            [zeros(n_L*m_F,n_L*m_F*i),eye(n_L*m_F),-H_M(n_L*m_F*i+1:n_L*m_F*(i+1),:)*Phi_M(PARAMS.m*i+1:PARAMS.m*(i+1),:)*Dummy_variable];
    end
    M_k= B_bar' / Y * B_bar;
    
    % Removing the last element (matrix) to fix the horizon
    H_M= H_M(1:end-(n_L*m_F),:);
    L_M= L_M(1:end-PARAMS.m,:);
    L_p_M= L_p_M(1:end-PARAMS.m,:);
    L_pp_M= L_pp_M(1:end-PARAMS.m,:);
...
Fault_slope= inf* ones( (n_L*m_F*(M+1)+PARAMS.m)*(n_L*(M+1))+1, 1 );
Fault_mag= inf* ones( (n_L*(M+1))+1, 1 );

% Detector threshold including the PH
T_RB= chi2inv(1 - PARAMS.C_REQ, n_L*m_F*(M+1));

% E matrix for only previous state faults
E= zeros( PARAMS.m, (n_L*m_F*(M+1)+PARAMS.m) );
E(:, end-PARAMS.m+1:end)= eye(PARAMS.m);

P_HMI_worst=0; % Initialization of integrity risk

% First term in integrity risk equation for every hypothesis (concatenated)
P_HMI_H= inf* ones((n_L*(M+1))+1,1);

% Worst-case fault slope for only previous state faults
Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m )= E'/(E*M_k*E')*E*A_k'*alpha;
Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m )=Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m )/norm(Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m ));

[Fault_mag(1), P_HMI_H(1)]= fminbnd(@(Fault_magnitude) -((1-cdf('Normal',PARAMS.alert_limit,alpha'*A_k*Fault_magnitude*Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m ),alpha'*P_Hat*alpha)+cdf('Normal',-PARAMS.alert_limit,alpha'*A_k*Fault_magnitude*Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m ),alpha'*P_Hat*alpha)) * cdf('Noncentral Chi-square',T_RB,m_F*n_L*(M+1),(Fault_magnitude^2)*(Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m ))'*B_bar'/Y*B_bar*(Fault_slope( 1 : n_L*m_F*(M+1)+PARAMS.m )))), -10, 10);
P_HMI_H(1)= -P_HMI_H(1);
P_HMI_worst=P_HMI_worst+P_HMI_H(1)/1000;
...
for i= 1:n_L*(M+1)
    ...
    Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) )=...
        E'/(E*M_k*E')*E*A_k'*alpha;
    Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) )=Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) )/norm( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) );
    
    [Fault_mag(i+1), P_HMI_H(i+1)]= fminbnd(@(Fault_magnitude) -((1-cdf('Normal',PARAMS.alert_limit,alpha'*A_k*Fault_magnitude*Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) ),alpha'*P_Hat*alpha)+cdf('Normal',-PARAMS.alert_limit,alpha'*A_k*Fault_magnitude*Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) ),alpha'*P_Hat*alpha)) * cdf('Noncentral Chi-square',T_RB,m_F*n_L*(M+1),(Fault_magnitude^2)*(Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) ))'*B_bar'/Y*B_bar*(Fault_slope( (n_L*m_F*(M+1)+PARAMS.m)*i+1 : (i+1)*(n_L*m_F*(M+1)+PARAMS.m) )))), -10, 10);
    P_HMI_H(i+1)= -P_HMI_H(i+1);
    P_HMI_worst=P_HMI_worst+P_HMI_H(i+1)/1000;
end
...
for i= 1:n_L
    Dummy_Variable_2 = Dummy_Variable_2 +...
        cdf( 'Chisquare' , max( (0.5*min_y_norm(i))^2 , (min_y_norm(i)-sqrt(PARAMS.T2))^2 ) );
end
